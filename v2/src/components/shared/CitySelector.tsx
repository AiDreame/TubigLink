...
import { ALL_SUPPORTED_CITIES, CITIES_BY_PROVINCE, PROVINCES_BY_ISLAND } from "@/lib/constants";
...
export function CitySelector({ variant = "compact", onChange }: CitySelectorProps) {
  ...
  const getProvinceForCity = (cityId: string): string => {
    const city = ALL_SUPPORTED_CITIES.find((c) => c.id === cityId);
    return city?.province || "Metro Manila";
  };

  // Build province groups with island separators
  const buildProvinceGroups = () => {
    const islandOrder = ["Luzon", "Visayas", "Mindanao"];
    const groups: { island: string; province: string; cities: typeof ALL_SUPPORTED_CITIES }[] = [];

    for (const island of islandOrder) {
      const provinces = PROVINCES_BY_ISLAND[island] || [];
      for (const province of provinces) {
        const provinceCities = CITIES_BY_PROVINCE[province] || [];
        const filtered = provinceCities.filter(
          (c) => searchQuery === "" || c.label.toLowerCase().includes(searchQuery.toLowerCase())
        );
        if (filtered.length > 0) {
          groups.push({ island, province, cities: filtered });
        }
      }
    }
    return groups;
  ...
  const provinceGroups = buildProvinceGroups();

  // Track last island for separator
  let lastIsland = "";
      ...
      {isOpen && (
        <div className="absolute left-0 top-full mt-1 w-72 bg-card rounded-2xl shadow-2xl border border-border z-50 overflow-hidden">
          ...
          <div className="max-h-72 overflow-y-auto p-2 space-y-1">
            ...
            {provinceGroups.map((group, idx) => {
              const showIslandHeader = group.island !== lastIsland;
              lastIsland = group.island;
              return (
                <div key={`${group.island}-${group.province}`}>
                  {showIslandHeader && (
                    <div className="px-2 py-1.5 text-[10px] font-bold uppercase tracking-wider text-muted-foreground/50 mt-1 first:mt-0">
                      {group.island}
                    </div>
                  )}
