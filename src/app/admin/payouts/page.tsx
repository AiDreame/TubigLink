...
export default function Payouts() {
  ...
  return <main className="space-y-6 p-8"><h1 className="text-3xl font-bold">Weekly payouts</h1>
    ...
    {rows.map(p => <details key={p.id} className="rounded border bg-white p-4">
      <summary className="cursor-pointer"><b>{p.station.name}</b> · {p.status} · {peso(p.netCentavos)} <span className="ml-4 text-sm text-slate-500">{destination(p)} · Gross {peso(p.grossCentavos)} · PayMongo fee {peso(p.processingFeeCentavos || 0)} · Commission {peso(p.commissionCentavos)} · Held {peso(p.heldCentavos)} · Adjustments {p.adjustmentCentavos < 0 ? `−${peso(-p.adjustmentCentavos)}` : peso(p.adjustmentCentavos)}{p.disbursementFeeCentavos ? ` · Transfer fee ${peso(p.disbursementFeeCentavos)}` : ""} · {p.periodStart.slice(0, 10)}–{p.periodEnd.slice(0, 10)}</span></summary>
      <div className="mt-4 space-y-2">
        {p.netCentavos === 0 && <p className="text-sm text-red-600">Net is below the ₱10 transfer fee (deducted from the payout) — the station receives nothing and no transfer can be sent.</p>}
