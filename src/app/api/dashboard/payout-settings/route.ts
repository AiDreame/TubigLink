...
import { encryptPayout, hashOtp, decryptPayout } from "@/lib/payout-security";
...
async function verify(userId:string, purpose:string, code:any){ if(typeof code!=="string"||!/^\d{6}$/.test(code)) return false; const row=await prisma.otpCode.findFirst({where:{userId,purpose,consumedAt:null},orderBy:{createdAt:"desc"}}); if(!row||row.attempts>=5||row.expiresAt<new Date()) return false; if(hashOtp(code)!==row.codeHash){await prisma.otpCode.update({where:{id:row.id},data:{attempts:{increment:1}}});return false;} await prisma.otpCode.update({where:{id:row.id},data:{consumedAt:new Date()}});return true; }
...
export async function POST(req:NextRequest){try{const body=await req.json();const a=await access(req,body);if(a.error)return a.error; if(!["ADD","EDIT"].includes(body.action)||!['BANK','GCASH'].includes(body.payoutMethod))return NextResponse.json({error:"Invalid payout details"},{status:400}); const n=String(body.accountNumber||"").replace(/\s/g,""); if(!/^\d{6,17}$/.test(n)||(body.payoutMethod==="BANK"&&!/^\d{12}$/.test(n)))return NextResponse.json({error:"Invalid account number"},{status:400}); if(!(await verify(a.user.id,`PAYOUT_${body.action}`,body.code)))return NextResponse.json({error:"Invalid, expired, or already used code"},{status:400}); const updated=await prisma.station.update({where:{id:a.station.id},data:{payoutMethod:body.payoutMethod,payoutAccountName:String(body.accountName||"").slice(0,120),payoutAccountLast4:n.slice(-4),payoutDetails:encryptPayout(JSON.stringify({accountNumber:n}))}});return NextResponse.json({success:true,data:masked(updated)});}catch(e){console.error(e);return NextResponse.json({error:"Failed"},{status:500});}}
export async function PATCH(req:NextRequest){return POST(req);}
export async function DELETE(req:NextRequest){try{const body=await req.json();const a=await access(req,body);if(a.error)return a.error;if(!(await verify(a.user.id,"PAYOUT_REMOVE",body.code)))return NextResponse.json({error:"Invalid, expired, or already used code"},{status:400});await prisma.station.update({where:{id:a.station.id},data:{payoutMethod:null,payoutAccountName:null,payoutAccountLast4:null,payoutDetails:null}});return NextResponse.json({success:true});}catch(e){return NextResponse.json({error:"Failed"},{status:500});}}
